...
export class ProductService {
  ...
  async unStash(
    ...
    if (candidate) {
      ...
      if (!stashed) return new HttpException(`Product ${name} not found`, 404);
    ...
    return new HttpException(
      `Warehouse with id: ${product.warehouseId} not found`,
      404,
    );
  }
  ...
  async getAll(): Promise<any> {
    ...
    if (products.length > 0) allProducts.products.push(products);

    if (stashed.length === 0)
      allProducts.stashed.push({ warning: 'Stashed products not found' });

    if (stashed.length > 0) allProducts.stashed.push(stashed);
  ...
  async remove(id: number): Promise<Product | HttpException> {
    ...
    if (candidate instanceof Product) await candidate.destroy();
  ...
  async moveTo(data: MoveDto, id: number): Promise<HttpException> {
    ...
    if (!product) return new HttpException('Product to move not found!', 404);

    if (data.stock > product.stock)
      return new HttpException(
        `You cant move more product when you have in warehouse`,
        409,
      );
  ...
  async getByWarehouseId(warehouseId: number) {
    const products = await this.productRepo.findAll({
      where: { warehouseId: warehouseId },
    });

    if (!products) return new HttpException('Products not found', 404);
